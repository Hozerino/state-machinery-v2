...
@onready var from_state: State = $".."
@export var to_state: State

func meets_trigger_requirements(input: InputData) -> bool:
	return false

# Q: As basic combos have one plain function, which is always called in some Move's check_relevance
...
# A: Well, for such a basic example as chaining consecutive strikes into series, it can be true.
